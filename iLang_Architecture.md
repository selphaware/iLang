...
## 4. GPT-5 Prompting Policy (Code-Only)

### 4.1 System Instruction (essentials)
- You output **code and machine-readable metadata only**. No prose, comments, or explanations.
- Emit a single **JSON envelope**. Files are included as explicit entries with `path`, `language`, and `body` fields.
- Obey the function signature and C ABI in §7. No network, no file I/O beyond function scope.
- Provide minimal, deterministic tests.
- Use portable standard libraries only; avoid OS-specific behavior unless requested.

### 4.2 Output Envelope Schema
Top-level response must be a JSON object. Example (truncated):

```json
{
  "version": "1",
  "files": [
    {"path": "example.f1.py", "language": "python", "body": "<code>"},
    {"path": "example.f1_shim.h", "language": "c", "body": "<c header>"},
    {"path": "example.f1_shim.c", "language": "c", "body": "<c shim impl>"},
    {"path": "tests/test_example_f1.py", "language": "python", "body": "<tests>"}
  ],
  "build": { "toolchain": "clang", "std": "c17", "flags": ["-O2","-Wall","-Werror"] },
  "signature": { "result": "int32", "inputs": ["int32","int32"] },
  "provenance": { "model": "gpt-5", "prompt_hash": "…", "deps": [] }
}
```

The runner **rejects** any response that does not parse or that contains text outside the envelope. Upon success, files are written into `.X/<func>/` (see §6).
