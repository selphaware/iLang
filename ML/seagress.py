...
def try_export_m2cgen(model_name, model, n_lags):
    ans = input("Generate C model with header via m2cgen? [y/N]: ").strip().lower()
    if ans != "y":
        return
    try:
        import m2cgen as m2c
        c_code = m2c.export_to_c(model)  # produces a C function: double score(const double * input);
...
#define MODEL_N_FEATURES {n_lags}
...
"""
        with open("model_generated.h", "w", encoding="utf-8") as f:
            f.write(h_code)
...
def main():
    ...
    try:
        scores = pd.DataFrame([{
            "model": model_name, "train_pct": train_pct,
            "mae": mae, "rmse": rmse, "r2": r2
        }])
        scores.to_csv("scores.csv", index=False)
        print("Saved scores -> scores.csv")
    except Exception:
        pass
    ...
    # Exports for C/C++ integration
    try_export_onnx(model_name, model, n_lags)
    try_export_m2cgen(model_name, model, n_lags)
